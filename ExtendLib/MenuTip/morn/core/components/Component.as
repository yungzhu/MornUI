...
package morn.core.components {
	...
	/**显示菜单时触发*/
	[Event(name="showMenu",type="morn.core.events.UIEvent")]
	/**隐藏菜单时触发*/
	[Event(name="hideMenu",type="morn.core.events.UIEvent")]
	...
	public class Component extends Sprite implements IComponent {
		...
		protected var _menuDelay:int = 0;
		...
		public function get menuDelay():int {
			return _menuDelay;
		}
		...
		public function set menuDelay(value:int):void {
			...
			if (_menuDelay > 0) {
				...
				removeEventListener(MouseEvent.MOUSE_DOWN, onMenuMouse);
				removeEventListener(MouseEvent.MOUSE_UP, onMenuMouse);
			}
		}
		...
		protected function onMenuMouse(e:MouseEvent):void {
			if (e.type == MouseEvent.MOUSE_DOWN && _menuDelay > 0) {
				App.timer.doOnce(_menuDelay, delayCallShowMenu);
			}else if (e.type == MouseEvent.CLICK) {
				App.timer.clearTimer(delayCallShowMenu);
				if(_menuDelay<=0){
					delayCallShowMenu();
				}
		...
		protected function delayCallShowMenu():void {
